...
def find_policy(T, N, B, u=np.sqrt):
    ...
    """
    P = eat_cake(T,N,B,u)[1]  #using our previous formula, define P 
    c = [P[T][0]]  #set the initial value of c which is the bottom left entry of P
    w = 1/N  #set w as the fraction 1/N
    for i in range(1,N):
        #for each column in P, add the value that corresponds to how many pieces we have left to c 
        c.append(P[i][int(c[i-1]/w)])
    return c
